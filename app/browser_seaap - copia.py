...
def ensure_playwright_browsers(log):
    ...
    try:
        # 1) Verificar si Playwright ya tiene Chromium disponible
        with sync_playwright() as p:
            exe = p.chromium.executable_path  # ruta que Playwright planea usar
            if exe and os.path.exists(exe):
                log(f"Playwright: Chromium ya está instalado. OK → {exe}")
                return True

        # 2) Si no existe, instalar desde la nube
        from playwright.__main__ import main as pw_cli_main
        log("Playwright: Chromium NO encontrado. Instalando desde la nube (esto puede tardar unos minutos)...")

        # Asegurar que la variable de entorno esté setada para este proceso
        os.environ["PLAYWRIGHT_BROWSERS_PATH"] = str(BROWSERS_DIR)

        old_argv = sys.argv[:]
        try:
            # Forzar la instalación de chromium
            sys.argv = ["playwright", "install", "chromium"]
            try:
                pw_cli_main()
            except SystemExit:
                # Playwright CLI suele hacer sys.exit(0) al terminar, lo ignoramos si es 0
                pass
        except Exception as e:
            log(f"[ERROR] Falló la instalación de Playwright: {e}")
            raise
        finally:
            sys.argv = old_argv
        ...
        with sync_playwright() as p:
            ...
            if exe and os.path.exists(exe):
                log(f"Playwright: instalación completa. OK → {exe}")
                return True

        raise RuntimeError("Playwright instaló Chromium pero no se encontró el ejecutable.")
...
def open_browser(headless, log):
    ensure_playwright_browsers(log)  # ✅ ahora solo instala si falta
...
def logout_seaap(page, log):
    ...
    try:
        # 1. Navegación directa a logout (más seguro)
        page.goto("http://seaap.minsa.gob.pe/web/session/logout", wait_until="domcontentloaded", timeout=20000)
        page.wait_for_timeout(2000)

        # 2. Verificar si ya estamos en login
        if page.locator("input[type=password]").count() > 0:
            log("[SEAAP] Sesión cerrada correctamente.")
            return True

        # 3. Si no funcionó, intentar vía menú (fallback)
        ...
        # 5. Fallback final: Borrar cookies
        log("[SEAAP][WARN] No se pudo cerrar sesión. Borrando cookies forzosamente.")
        page.context.clear_cookies()
        page.goto("http://seaap.minsa.gob.pe/web/login", wait_until="domcontentloaded")
        return True
...
def login_seaap(page, user, pwd, log):
    ...
    if page.locator("input[type=password]").count() == 0:
        ...
        # Volver al padrón tras logout
        page.goto(PADRON_URL, wait_until="domcontentloaded", timeout=60000)
    ...
    if page.locator("input[type=password]").count() > 0:
        ...
        try:
             # Esperamos hasta que la URL ya no tenga 'login' O aparezca el navbar de Odoo
...
def cerrar_todos_los_modales(page, log, max_loops=10):
    ...
    try:
        for ciclo in range(1, max_loops + 1):
            ...
            modales = page.locator("div.modal.in[role='dialog']")
            count = modales.count()
            ...
            btn_aceptar = modal.locator("button:has-text('Aceptar')")
...
def presionar_guardar(page, log):
    btn = page.locator("button.o_form_button_save")

    for _ in range(20):
        if btn.count():
            btn.first.click()
            log("[SEAAP] Guardado exitoso.")
            page.wait_for_timeout(1000)
            return True
        page.wait_for_timeout(200)
...
def seleccionar_periodo(page, log):
    ...
    for i in range(rows.count()):
        ...
        if chk.count():
            row.locator("td[data-field='documento_tipo']").click()
            page.wait_for_timeout(500)
            return True
    return False
...
def seleccionar_fila_periodo_manual(page, periodo_manual, log):
    ...
    for i in range(total):
        ...
        if periodo == periodo_seaap:
            row.locator("td[data-field='documento_tipo']").click()
            page.wait_for_timeout(500)
            log("[SEAAP] Fila seleccionada correctamente.")
            return True
...
def run_seaap_flow_for_account(
    ...
    p, ctx, page = open_browser(headless, log)
    ...
    try:
        ...
        if registros_fallidos:
            ...
            nuevos = [r for r in registros if r["dni"] in [f["dni"] for f in registros_fallidos]]

            run_seaap_flow_for_account(account, nuevos, log, periodo_manual, headless)
